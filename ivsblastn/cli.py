...
from .reference import clean_reference_ivs, preprocess_reference
...
DEFAULT_MIN_IVS_LEN = 25
DEFAULT_MAX_IVS_LEN = 2000
...
def slurm_run_args(args: argparse.Namespace) -> List[str]:
    ...
    forwarded = [
        ...
        ("--min-ivs-len", args.min_ivs_len),
        ("--max-ivs-len", args.max_ivs_len),
...
def add_run_args(parser: argparse.ArgumentParser) -> None:
    ...
    required.add_argument("--db", required=True, type=Path, help="Initialized BLAST database prefix from `ivsBLASTn init-reference`.")
    required.add_argument("--taxonomy", required=True, type=Path, help="Initialized reference taxonomy TSV from `ivsBLASTn init-reference`.")
    required.add_argument("--outdir", required=True, type=Path, help="Output directory. No default.")

    blast_input = parser.add_argument_group("BLAST input")
    blast_input.add_argument("--blast", default=None, type=Path, help="Optional existing BLASTN outfmt 6 table. If omitted, ivsBLASTn runs BLASTN against --db.")
    ...
    filters = parser.add_argument_group("HSP filters and BLAST settings")
    ...
    ivs = parser.add_argument_group("Candidate intervening sequence geometry")
    ivs.add_argument("--min-ivs-len", default=DEFAULT_MIN_IVS_LEN, type=nonnegative_int, help=f"Minimum query gap size. Default: {DEFAULT_MIN_IVS_LEN} bp.")
    ivs.add_argument("--max-ivs-len", default=DEFAULT_MAX_IVS_LEN, type=positive_int, help=f"Maximum query gap size. Default: {DEFAULT_MAX_IVS_LEN} bp.")
    ivs.add_argument("--max-ref-gap", default=DEFAULT_MAX_REF_GAP, type=nonnegative_int, help=f"Maximum absolute reference gap/overlap. Default: {DEFAULT_MAX_REF_GAP} bp.")
    ivs.add_argument("--max-query-overlap", default=DEFAULT_MAX_QUERY_OVERLAP, type=nonnegative_int, help=f"Maximum allowed query HSP overlap. Default: {DEFAULT_MAX_QUERY_OVERLAP} bp.")
    ivs.add_argument("--breakpoint-window", default=DEFAULT_BREAKPOINT_WINDOW, type=nonnegative_int, help=f"Breakpoint clustering window. Default: {DEFAULT_BREAKPOINT_WINDOW} bp.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init_self_clean.add_argument("--clean-ref-ivs", action="store_true", help="Self-BLAST reference and remove candidate IVSs before final DB creation.")
    ...
    init_geometry = init_ref_parser.add_argument_group("Self-cleaning intervening sequence geometry")
    init_geometry.add_argument("--min-ivs-len", default=DEFAULT_MIN_IVS_LEN, type=nonnegative_int, help=f"Minimum query gap size for optional self-cleaning. Default: {DEFAULT_MIN_IVS_LEN} bp.")
    init_geometry.add_argument("--max-ivs-len", default=DEFAULT_MAX_IVS_LEN, type=positive_int, help=f"Maximum query gap size for optional self-cleaning. Default: {DEFAULT_MAX_IVS_LEN} bp.")
    ...
    submit_inputs.add_argument("--taxonomy", required=True, type=Path, help="Reference taxonomy TSV shared by all chunks.")
    ...
    submit_detection.add_argument("--min-ivs-len", default=DEFAULT_MIN_IVS_LEN, type=nonnegative_int, help=f"Minimum query gap size forwarded to each run. Default: {DEFAULT_MIN_IVS_LEN} bp.")
    submit_detection.add_argument("--max-ivs-len", default=DEFAULT_MAX_IVS_LEN, type=positive_int, help=f"Maximum query gap size forwarded to each run. Default: {DEFAULT_MAX_IVS_LEN} bp.")
...
def validate_run_args(args: argparse.Namespace) -> None:
    ...
    if args.min_ivs_len > args.max_ivs_len:
        raise ValueError("--min-ivs-len must be <= --max-ivs-len")


def run_pipeline(args: argparse.Namespace) -> int:
    """Run IVS detection against an initialized reference database."""
...
def setup_reference_output_paths(args: argparse.Namespace) -> None:
    ...
    args.ref_self_clean_ivs_fa = args.results_dir / "reference_self_clean.ivs.fa"
...
def init_reference_command(args: argparse.Namespace) -> int:
    ...
    if args.clean_ref_ivs:
        ref_fa, taxonomy_tsv, db_prefix = clean_reference_ivs(args, ref_fa, taxonomy_tsv, db_prefix)
    ...
    with manifest.open("wt", encoding="utf-8") as handle:
        ...
        if args.clean_ref_ivs:
            ...
            print(f"reference_ivs_fasta\t{args.ref_self_clean_ivs_fa.resolve()}", file=handle)
    ...
    if args.clean_ref_ivs:
        ...
        CONSOLE.print(f"  Reference IVSs:    {args.ref_self_clean_ivs_fa}")
